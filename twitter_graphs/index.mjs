...
const { DISCORD_TWITTER_GRAPHS_WEBHOOK, X_USERNAMES } = process.env;
if (!DISCORD_TWITTER_GRAPHS_WEBHOOK) {
  console.error("Missing env DISCORD_TWITTER_GRAPHS_WEBHOOK");
  process.exit(1);
}

const webhook = new WebhookClient({ url: DISCORD_TWITTER_GRAPHS_WEBHOOK });
    ...
    for (const username of users) {
      try {
        ...
        let newLinks = links.filter((link) => {
          ...
          if (isNew) console.log(`New link for ${username}:`, n);
          return isNew;
        });
        if (!newLinks.length) continue;

        // dedupe and send oldest-first
